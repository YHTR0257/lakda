...
_JA_KG_TRIPLET_EXTRACT_PROMPT = PromptTemplate(
    ...
    "---------------------\n"
    "例:\n"
    "テキスト: Python は汎用プログラミング言語であり、1991年にグイド・ヴァンロッサムが開発しました。\n"
    "トリプレット:\n"
    "(Python, is, プログラミング言語)\n"
    "(Python, 開発者, グイド・ヴァンロッサム)\n"
    "(Python, 開発年, 1991年)\n"
    "テキスト: ファイル命名規則では、snake_case と kebab-case を組み合わせることで視認性が向上します。\n"
    "トリプレット:\n"
...
class IndexPipeline:
    ...
    def run(
        ...
        logger.info(
            "[pipeline] MarkdownNodeParser + HeadingContextInjector + SentenceSplitter(chunk_size=%d, chunk_overlap=%d) ready (%.2fs)",
            chunk_size, chunk_overlap, time.perf_counter() - t0,
        )

        logger.info("[pipeline] PropertyGraphIndex.from_documents START — LLM extraction + Neo4j write")
        ...
        logger.info("[pipeline] PropertyGraphIndex.from_documents DONE elapsed=%.2fs", time.perf_counter() - t1)
