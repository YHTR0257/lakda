...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "db: データベース接続テスト（--run-dbで実行）"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_db = pytest.mark.skip(reason="DBテストをスキップ（--run-dbで実行）")
    for item in items:
        if "llm_api" in item.keywords:
            item.add_marker(skip_llm_api)
        if "db" in item.keywords and not config.getoption("--run-db"):
