...
| FT13       | LLM管理機能                    | Backend<br>LLM      | LLMモデルの切り替え・管理                                                   | モデル選択設定                                    | 選択されたLLMモデル                                                    |
...
- **説明**: システムで使用するLLMモデルを切り替え・管理する機能を提供します。これにより、異なるドメインやタスクに最適なモデルを選択できます。
- **実装コンポーネント**: Backend (`backend/src/lakda/core/llm/`), Frontend (管理UI)
- **機能**:
  - 利用可能なLLMモデルの一覧表示
  - モデルの選択・切り替え
  - モデルのステータス監視
